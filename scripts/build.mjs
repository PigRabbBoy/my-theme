...
//   out: warp/pigrabb_dark.yaml, warp/pigrabb_light.yaml, zed/pigrabb.json
...
function warpTheme(get, ansi, variant) {
  ...
  const bg = toHex(get(dark ? 'night.bg' : '--bg-surface'));
  const fg = toHex(get(dark ? 'night.fg' : '--text-primary'));
  const accent = toHex(get(dark ? 'night.pink' : '--color-pink-700'));
  const label = `warp/${variant}`;
  check(`${label} foreground`, fg, bg, 4.5);
  check(`${label} cursor`, accent, bg, 3);
  checkAnsi(label, ansi, bg);
